...
默认只写**上一版本号 → 目标版本号**这一段差。上一版本取本仓库已发布的最新稳定版本（最新稳定标签，或 `CHANGELOG` 里目标版本的前一小节），不要把更早版本的累计变化再写一遍，也不要从上游完整历史复制条目。
...
- 目标版本已有小节：原位覆写这一节，不要在节内或节后加条目
- 目标版本没有小节：在文件顶部新增唯一小节
- 不得改写、插入或追加更早版本的小节，除非用户明确要求改旧日志
- 不得把本次变化补记到旧版本下面

`.github/CHANGELOG.md` 可以保留历史小节，但只动目标版本那一节。`changelog.txt` 同步同一目标版本，写得更短，供构建脚本校验。

## 操作

1. 先从 GitHub Release 查询上一个已发布的稳定标签；查询失败时不要猜测。用上一版本号与目标版本号框定更新日志，不要扩大范围。
2. 按上一节覆写目标版本的唯一 `## <版本>` 小节，并同步 `changelog.txt` 的同版本条目。
