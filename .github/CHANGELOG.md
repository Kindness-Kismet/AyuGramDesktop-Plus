...
- 会话列表的选中态改为内缩圆角矩形样式。
- 侧栏文件夹按钮的选中态改为内缩圆角并添加强调条。
- 移除错误的 7.2.10 存储兼容代码，避免版本判断异常。
- macOS 恢复为 Intel 与 Apple 芯片分别提供安装包和更新包。
- 修复 Windows ARM64 构建的链接问题。
...
- 修复消息气泡描边可能被相邻消息擦除，以及描边与左右拖尾衔接异常的问题。
- macOS 与 Linux 现在可以从本仓库接收自动更新；macOS 使用同时支持 Intel 与 Apple 芯片的通用安装包，并为两种架构提供更新包。
