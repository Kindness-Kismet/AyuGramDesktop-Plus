...
void BuildQoLToggles(SectionBuilder &builder, AyuSectionBuilder &ayu) {
	...
	ayu.addSettingToggle({
		.id = u"ayu/autoSpaceSending"_q,
		.title = tr::ayu_AutoSpaceSending(),
		.getter = &AyuSettings::autoSpaceSending,
		.setter = &AyuSettings::setAutoSpaceSending,
	});
	ayu.addSettingToggle({
		.id = u"ayu/autoSpaceEditing"_q,
		.title = tr::ayu_AutoSpaceEditing(),
		.getter = &AyuSettings::autoSpaceEditing,
		.setter = &AyuSettings::setAutoSpaceEditing,
	});
	const auto autoSpaceReceivingButton = builder.addButton({
		.id = u"ayu/autoSpaceReceiving"_q,
		.title = tr::ayu_AutoSpaceReceiving(),
		.st = &st::settingsButtonNoIcon,
	...
	if (autoSpaceReceivingButton) {
		autoSpaceReceivingButton->toggledValue(
			...
			[=](bool enabled) {
				AyuSettings::getInstance().setAutoSpaceReceiving(enabled);
				ShowRestartPrompt(controller);
			},
