...
void BuildOtherThings(SectionBuilder &builder) {
	...
	if (AyuSettings::getInstance().devFeaturesEnabled()) {
		builder.addButton({
			.id = u"ayu/restoreHidden"_q,
			.title = tr::ayu_RestoreHiddenFeatures(),
			.icon = { &st::menuIconRestore },
			.onClick = [=] {
				AyuSettings::getInstance().setDevFeaturesEnabled(false);
				controller->showToast(tr::ayu_DevFeaturesRestored(tr::now));
			},
		});
	}
