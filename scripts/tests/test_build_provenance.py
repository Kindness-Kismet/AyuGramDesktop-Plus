...
class ArtifactProvenanceTests(unittest.TestCase):
    ...
    def test_split_macos_manifests_cover_separate_update_channels(self):
        self.add_required_targets()
        x64 = json.loads(
            (self.root / "macos/provenance-macos-x64.json").read_text(encoding="utf-8")
        )
        arm64 = json.loads(
            (self.root / "macos/provenance-macos-arm64.json").read_text(encoding="utf-8")
        )
        self.assertEqual(
