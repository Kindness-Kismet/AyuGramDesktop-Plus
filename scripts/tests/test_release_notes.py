...
"""
        self.assertEqual(
            extract_release_notes(changelog, "7.2.10"),
            "## 7.2.10\n\n- 适配官方 Telegram Desktop 7.2.9。\n- 修复视频播放问题。\n",
        )
...
- 旧内容。
"""
        self.assertIn("### 修复", extract_release_notes(changelog, "7.2.10"))

    def test_missing_version_is_rejected(self):
        with self.assertRaisesRegex(ValueError, "缺少版本 7.2.10"):
    ...
    def test_cli_writes_only_selected_version(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(
                sys,
                "argv",
                ["release_notes.py", str(changelog), "7.2.10", str(output)],
            ):
                main()
            self.assertEqual(
